...
main()
{
	...
	float dx0[90000],dy0[90000],dpval[90000],dnval[90000],dum;
	float mean,sdev,skew,kurt,min,max,g1,d1,l1,xi1,nvar,qval;
	double ppx,ppx2,ppx3,winprop;
	...
	for(j=0;j<ndata;++j) {
		pvcalc(dx0[j],dy0[j],x1,y1,n,iwin,&ppx,&ppx2,&ppx3);
		fprintf(out,"%f %f %f %f %f\n",dx0[j],dy0[j],ppx,ppx2,ppx3);
	}

}


void pvcalc(het,fst,hvec,fvec,len,win,pval,pval2,pval3)
float het;
float fst;
float hvec[];
float fvec[];
int len;
int win;
double *pval,*pval2,*pval3;

{
	double *dummy,pvala,pvalb;
	int j,i,ic,winstart,winend,ipos,extra;
	...
	dsort('a',ic,dummy);
	for(j=0;j<ic;++j){
		if(!(dummy[j] < fst || fabs(dummy[j] - fst) < 0.00001))break;
	}
	*pval = (double)(j)/(ic);


	for(j=0;j<ic;++j){
		if((fabs(dummy[j] - fst) < 0.00001) || dummy[j] > fst)break;
	}
	*pval2 = (double)(ic -j)/(ic);

	if(*pval > 0.5){
		if(1.0-(*pval2) < *pval) *pval3 = 1.0 - *pval2;
		else *pval3 = *pval;
	}
	else{
		if(1.0-(*pval2) > *pval) *pval3 = 1.0 - *pval2;
